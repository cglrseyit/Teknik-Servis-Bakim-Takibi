...
async function create(req, res) {
  const { name, brand, model, supplier, status, notes, maintenance_period, maintenance_start_date, maintenance_start_day, quantity, parent_id, units } = req.body;
          ...
          for (const child of children) {
            const { rows: planRows } = await pool.query(
              `INSERT INTO maintenance_plans
                 (equipment_id, title, frequency_type, frequency_days, advance_notice_days, start_date, is_one_time, is_active, target_month, target_day)
               VALUES ($1,$2,$3,$4,$5,$6,false,true,$7,$8) RETURNING *`,
              [child.id, `Periyodik Bakım`, freq.frequency_type, freq.frequency_days, 3, startDate, targetMonth, day]
            );
        ...
        for (const eqId of planTargets) {
          const { rows: planRows } = await pool.query(
            `INSERT INTO maintenance_plans
               (equipment_id, title, frequency_type, frequency_days, advance_notice_days, start_date, is_one_time, is_active, target_month, target_day)
             VALUES ($1,$2,$3,$4,$5,$6,false,true,$7,$8) RETURNING *`,
            [eqId, `Periyodik Bakım`, freq.frequency_type, freq.frequency_days, 3, startDate, targetMonth, day]
          );
          await generateTasksForPlan(planRows[0], 365);
        }
      } catch (planErr) {
